    ...
    for test_sentence in test:
        ...
        for word in test_sentence:
            if word in bag_of_words:
                maximum = (max(bag_of_words[word].items(), key=lambda k: k[1]))[0]
                current_sentence.append((word,maximum))
            else:
                current_sentence.append((word,'unseen'))
        predicts.append(current_sentence)
